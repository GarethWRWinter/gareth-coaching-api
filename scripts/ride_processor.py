...
from scripts.ride_database import store_ride  # ensure RideStorageError is optional
from scripts.sanitize import sanitize

def process_latest_fit_file(filepath: str, ftp: float) -> Dict:
    """
    Process a .FIT file and compute ride metrics.
    Returns a sanitized dictionary with ride summary data (ready for storage).
    """
    df: pd.DataFrame = parse_fit(filepath)
    if df.empty:
        raise ValueError(f"No data found in FIT file at {filepath}")

    ride_metrics: Dict = calculate_ride_metrics(df, ftp)

    # Make all primitive types JSON compatible
